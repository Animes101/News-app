  ...
  return (
    <div className='my-20'>
        <div className="flex flex-col p-6 space-y-6 overflow-hidden rounded-lg shadow-md dark:bg-gray-50 dark:text-gray-800">
            ...
            <div className='space-x-2'>
                {tags?.map(item => (
                    <span key={item} className="inline-block px-3 py-1 text-sm font-semibold text-gray-800 bg-gray-200 rounded-full dark:bg-gray-700 dark:text-gray-300">
                        {item}
                    </span>
                ))}
            </div>
            <div>
                {rating && <Rating style={{ maxWidth: 250 }} value={rating.number} />}
                <h2 className='text-gray-500'>{rating?.badge}</h2>
            </div>
        </div>
    </div>
  )
